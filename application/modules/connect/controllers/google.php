...
class Google extends CI_Controller {
	...
	function index() {
		...
		$providers = $this->config->item('google','social_media_providers');
		...
		if($this->config->item('social_media_enabled')===TRUE && $providers['enabled']===TRUE ) {
			...
			if ($this->input->get('janrain_nonce')) {
				...
				$response = $consumer->complete(site_url('connect/google'));
			...
			header("Location: ".$auth_request->redirectURL(base_url(), site_url('connect/google')));
		}
		else {
