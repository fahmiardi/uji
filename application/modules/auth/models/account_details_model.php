...
class Account_details_model extends CI_Model {
	...
	function update($account_id, $attributes = array())
	{
		...
		{
			$this->load->model('auth/ref_country_model');
		...
		if (isset($attributes['language'])) 
		{
			...
			if (isset($language[0]))
			{
				$this->load->model('auth/ref_language_model');
		...
		{
			$this->load->model('auth/ref_zoneinfo_model');
		...
		if ( ! isset($attributes['country']))
		{
			$this->load->model('auth/ref_iptocountry_model');
